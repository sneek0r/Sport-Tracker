...
public class RecordUI extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		locationListener = new SportTrackerLocationListener(this, new Record(
				extras.getCharSequence("profile").toString()));
	}
...
//			stopDate = new Date(stopMillis);
		...
		recordinfo_intent.putExtra("time", recorded_time);
		recordinfo_intent.putExtra("distance", locationListener.record.distance);
		recordinfo_intent.putExtra("avarage_speed", locationListener.record.avarageSpeed);
	...
	public void updateFields(Record record) {
		if (!this.isFinishing()) {
			Location lastLocation = ((LinkedList<Waypoint>) record.track)
					.getLast().getLocation();
